...
describe("JobApplicationService - applyForJob", () => {
  ...
  it("should throw a Conflict error if the application already exists", async () => {
    ...
    (JobApplication.findOne as jest.Mock).mockResolvedValue(
      existingApplication
    ); // Simulate existing application

    await expect(
      jobApplicationService.applyForJob(applicationPayload)
  ...
  it("should handle unexpected errors", async () => {
    ...
    (JobApplication.findOne as jest.Mock).mockRejectedValue(
  ...
  describe("getApplicationsByJobId", () => {
    it('should fetch applications for a valid job ID', async () => {
      const mockJobId = "66f6541cb19387f46b2c2613"; // Example job ID
      const mockApplications = [
        { job_id: new Types.ObjectId(mockJobId), job_seeker_id: new Types.ObjectId("66f6541cb19387f46b2c2614"), status: "applied" },
      ];
    
      // Mock the find method to return the expected applications
      jest.spyOn(JobApplication, 'find').mockResolvedValueOnce(mockApplications);
    
      const applications = await jobApplicationService.getApplicationsByJobId(mockJobId);
    
      expect(applications).toEqual(mockApplications);
      expect(JobApplication.find).toHaveBeenCalledWith({ job_id: new Types.ObjectId(mockJobId) });
      expect(console.log).toHaveBeenCalledWith("Fetching applications for job ID:", mockJobId);
    });
    ...
    it("should throw an error for an invalid job ID", async () => {
      ...
      await expect(
        jobApplicationService.getApplicationsByJobId(invalidJobId)
  ...
  describe("updateApplicationStatus", () => {
    it("should update the application status successfully", async () => {
      ...
      console.log(
        "Mock application data for updating status:",
        updatedApplication
      );
      ...
      console.log(
        `Application ${applicationId} status updated to ${JobApplicationStatus.APPLIED}`
      );
    ...
    it("should return null if the application is not found", async () => {
      ...
      console.log(
        `Application ${applicationId} not found during status update`
      );
    ...
    it("should handle errors gracefully", async () => {
      ...
      console.log(
        `Failed to update status for application ${applicationId}: Database error`
      );
